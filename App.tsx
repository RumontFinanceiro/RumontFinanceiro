...
const App: React.FC = () => {
  ...
  // Inicialização do mês usando data local para evitar conflitos UTC no Cloudflare
  ...
  const handleSaveCreditSale = async (s: StoreCreditSale, t?: Transaction | null) => {
    try {
      ...
      alert('Erro ao salvar dados no servidor: ' + err.message);
      throw err;
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center">
        ...
        <p className="text-white mt-4 font-bold tracking-widest uppercase text-xs italic">Sincronizando Rumont Cloud...</p>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl p-10 border border-slate-200">
          <div className="text-center mb-10">
            ...
            <p className="text-slate-400 font-medium uppercase text-[10px] tracking-[0.2em] mt-2">Controle Financeiro</p>
          </div>
          <form onSubmit={(e) => {
